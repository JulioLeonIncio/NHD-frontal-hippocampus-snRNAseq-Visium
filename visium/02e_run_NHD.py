#!/usr/bin/env python
# =============================================================================
# 02e_run_NHD.py — NHD cell2location.
...
# Outputs: Visium/cell2location/c2l_MAIN/{ref_signatures.csv, sp_model/,
#          q05_cell_abundance.csv, visium_c2l.h5ad}
...
log("loading sc_ref mtx bundle")
adata_ref = load_mtx_bundle(os.path.join(C2L, "sc_ref"))
...
log(f"reference after gene filter: {adata_ref.shape}")

RegressionModel.setup_anndata(adata=adata_ref, labels_key=LABEL_KEY, batch_key=BATCH_KEY)
mod = RegressionModel(adata_ref)
log(f"training reference regression ({REF_EPOCHS} epochs, CPU)")
mod.train(max_epochs=REF_EPOCHS, accelerator="cpu")
adata_ref = mod.export_posterior(
    adata_ref, sample_kwargs={"num_samples": 1000, "accelerator": "cpu"})

if "means_per_cluster_mu_fg" in adata_ref.varm.keys():
    inf_aver = adata_ref.varm["means_per_cluster_mu_fg"][
        [f"means_per_cluster_mu_fg_{c}"
         for c in adata_ref.uns["mod"]["factor_names"]]].copy()
else:
    inf_aver = adata_ref.var[[f"means_per_cluster_mu_fg_{c}"
         for c in adata_ref.uns["mod"]["factor_names"]]].copy()
inf_aver.columns = adata_ref.uns["mod"]["factor_names"]
inf_aver.to_csv(os.path.join(C2L, "ref_signatures.csv"))
